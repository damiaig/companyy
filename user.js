...
if (profileForm) {
    profileForm.addEventListener("submit", async function (e) {
        ...
        const newProfilePic = profilePictureInput.files[0];
        if (newProfilePic) {
...
async function loadUserInBox(userId) {
  ...
  try {
      ...
      let anyUnreadMessages = false;
      let pendingChecks = usersData.length; // Track pending checks
      ...
      usersData.forEach((inboxUser) => {
          ...
          onValue(unreadMessagesRef, (snapshot) => {
              ...
              snapshot.forEach((messageSnapshot) => {
                  ...
                  if (!messageData.read) unreadCount++;
              });
              ...
              pendingChecks--; // One less user left to check

              // ✅ Update UI only after all users are checked
              if (pendingChecks === 0) {
                  if (anyUnreadMessages) {
                      unreadCounter.textContent = totalUnreadCount; // Show total unread count
          ...
          }, { onlyOnce: true }); // Ensures listener runs only once per user
      });
  } catch (error) {
