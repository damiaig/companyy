...
const unreadGroupCounter = document.querySelector(".unread-group-counter");
document.addEventListener('DOMContentLoaded', async () => {
await loadUserInBox(userId);
await loadGroupInBox(userId); 
await loadMeetingInBox(userId)// Load inbox users on page load
});
async function loadUserInBox(userId) {
if (!userId) {
console.error("User ID is not defined. Cannot load inbox.");
return;
}
...
if (!inboxSnapshot.empty) {
    inboxSnapshot.forEach((doc) => {
        ...
        if (userData) {
            usersData.push({
                id: doc.id, // Document ID for the user
                name: userData.name || "Unknown User", // Fallback for missing names
            });
        }
    });
} else {
    console.log("No inbox data found.");
}

// Log the fetched users
console.log("Inbox Users:", usersData);

let totalUnreadCount = 0; // Track total unread messages across all users
let anyUnreadMessages = false; // Flag to track if there are any unread messages

// Check for unread messages for each user in RTDB
usersData.forEach((inboxUser) => {
    const unreadMessagesRef = dbRef(rtdb, `users/${inboxUser.id}/inbox/${userId}/messages`);
    console.log(unreadMessagesRef);
    ...
    onValue(unreadMessagesRef, (snapshot) => {
        ...
        if (unreadCount > 0) {
            console.log(`${inboxUser.name} (${inboxUser.id}) has ${unreadCount} unread messages.`);
            totalUnreadCount += unreadCount; // Add to the total unread count
            anyUnreadMessages = true; // There are unread messages
        } else {
            console.log(`${inboxUser.name} (${inboxUser.id}) has no unread messages.`);
        }

        // Show or hide the unread message counter based on totalUnreadCount
        if (anyUnreadMessages) {
            unreadCounter.textContent = ''; // Display the total unread count
            unreadCounter.style.display = 'block'; // Show counter
        } else {
            unreadCounter.textContent = ''; // Clear the counter text
            unreadCounter.style.display = 'none'; // Hide counter if no unread messages
        }
    });
});
} catch (error) {
...
async function loadGroupInBox(userId) {
if (!userId) {
console.error("User ID is not defined. Cannot load group inbox.");
return;
}
...
let totalUnreadCount = 0; // Track total unread messages across all groups
let anyUnreadMessages = false; // Flag to track if there are any unread messages
...
if (!groupsSnapshot.empty) {
    groupsSnapshot.forEach((doc) => {
        ...
        if (groupData.members && groupData.members.includes(userId)) {
            groupsData.push({
                id: doc.id, // Group ID
                name: groupData.name || "Unnamed Group", // Fallback for missing names
            });
        }
    });
} else {
...
console.log("Groups the user belongs to:", groupsData);

// Monitor unread messages for each group
groupsData.forEach((group) => {
    const unreadMessagesRef = dbRef(rtdb, `users/${userId}/inbox/${group.id}/messages`);
    onValue(unreadMessagesRef, (snapshot) => {
        let unreadCount = 0;
        ...
        if (unreadCount > 0) {
            console.log(`${group.name} (${group.id}) has ${unreadCount} unread messages.`);
            totalUnreadCount += unreadCount; // Add to the total unread count
            anyUnreadMessages = true; // There are unread messages
        } else {
            console.log(`${group.name} (${group.id}) has no unread messages.`);
        }

        // Update the unread message counter display
        if (anyUnreadMessages) {
            unreadCounter.textContent = ''; // Display the total unread count
            unreadCounter.style.display = 'block'; // Show counter
        } else {
...
console.error("Error loading group inbox:", error);
}
}
...
document.querySelectorAll(".posttt").forEach(postButton => {
  postButton.addEventListener("click", async () => {
    ...
    try {
      ...
      const announcementsRef = collection(db, "announcements"); // Firestore Collection Reference
      ...
      if (file) {
        // Post with image
        desImage = description; // Image post description
        desText = ""; // No text description for this type
      } else {
        // Post without image
        desImage = ""; // No image post description
        desText = descriptionWithoutImage; // Text-only post description
      }

      if (file) {
        // Generate a unique name for the file by appending a timestamp or unique identifier
      ...
      // Get the author ID from Firebase Authentication
      const authorId = userId; // Fetch the author's ID

      // Save post details in Firestore
      const newPost = {
        id: "",                      // Empty ID field to be updated after document creation
        title: title || null,        // Include title only if available
        desImage,                    // Image description
        desText,                     // Text-only description
        postType,
        imageUrl,
        likeCount: [],               // Initialize like count to 0
        timestamp: new Date(),
        authorId,                    // Add the author ID
      };

      // Create a new document for the post
      const newDocRef = doc(announcementsRef); // Generate a new document reference
      await setDoc(newDocRef, newPost); // Add the post to Firestore

      // After the post is created, update the 'id' field with the document's ID
      await updateDoc(newDocRef, {
        id: newDocRef.id, // Assign the document ID to the 'id' field
      });

      // Initialize an empty sub-collection for comments
      const commentsRef = collection(newDocRef, "comments");
      await setDoc(doc(commentsRef, "template"), {
        placeholder: true,
      }); // Add a placeholder comment or keep the collection empty
...
if (postElement) console.log("Post ID:", postElement.getAttribute("data-id"));
...
closingButton.addEventListener("click", () => {
    ...
    // Hide the modal
      ...
      sendCommentBtn.removeEventListener("click", sendCommentHandler); // Prevent duplicate listeners
            ...
            commentsHTML += `
                ...
                <span class="comment-time">${timeAgo}</span>
                    ...
                    <div class="user-profile-comment">
                        ...
                        <h2 class="user-name-comment">${userName}</h2>
                    </div>
                    <p class="comment-p">${commentData.text}</p>
                </div>
            `;
        }
