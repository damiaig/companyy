...
async function searchUsers(searchTerm) {
    ...
    // Get all users currently in the inbox
    const inboxUserIds = Array.from(inboxContainer.querySelectorAll('.user-inbox')).map(
        item => item.dataset.userId
    );

    // Display inbox users in the search results
    inboxUserIds.forEach(userId => {
        const user = allUsers.find(u => u.id === userId);  // Find the user data by ID
        if (user) {
            displaySearchResult(user.id, user.name);  // Display user from inbox
        }
    });
    ...
    const filteredUsers = allUsers.filter(user => 
        ...
        !inboxUserIds.includes(user.id) // Exclude users already in inbox
    );

    // Display filtered users in the search results
    filteredUsers.forEach(user => displaySearchResult(user.id, user.name));

    // Show or hide search results based on the number of items
